...
public:
  ...
  virtual const Key& floor(const Key& k) const = 0;   // largest key <= k
  virtual const Key& ceiling(const Key& k) const = 0; // smallest key >= k
  virtual size_type rank(const Key& k) const = 0; // number of keys < k
  virtual const Key& select(size_type r) const = 0; // key of rank r
  virtual size_type size(const Key& lo, const Key& hi) const = 0; // number of
                                                           // keys in [lo, hi]
